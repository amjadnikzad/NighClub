...
export class GamesRepository {
  ...
  async createGame({gameID,userID,gameMode}:CreateGameData):Promise<Game> {
    ...
    this.logger.log(`Creating New Game With This Info ${JSON.stringify(initilizedGame, null, 2)}`);
    ...
    try{
        ...
        .multi([
            ['send_command','JSON.SET', key, '.',JSON.stringify(initilizedGame)],
            ['expire', key, '100000']
        ])
        .exec();
  ...
  async addPlayer ({gameID,name,userID}:AddPlayerData): Promise<Game> {
    ...
    try {
        ...
        const gameJSON = await this.redisClient.send_command('JSON.GET',key,'.');

        const game = JSON.parse(gameJSON) as Game;
        this.logger.debug(`Current Players for this Game:${gameID}:`,game.players);
        
        return game;
    } catch(e){
        this.logger.error(`Failed to add player with playerID/name:${userID}/${name} to game: ${gameID}`);
        throw e;
    }
  }
}
